...
class SceneBuilder:
    ...
    def build_parts(self):
        ...
        if self.cfg['task_number'] == 3:
            ...
            # 填充 parts_prim_paths 供 get_parts_world_poses() 使用
            # 原始 Part_A/Part_B 也是独立物体，需一并加入
            self.parts_prim_paths = ["/Root/Part_A"] + list(target_paths_A) + ["/Root/Part_B"] + list(target_paths_B)
            self._initial_parts_prim_paths = list(self.parts_prim_paths)
            self._rigid_body_paths = []
            self._parts_rigid_prims = []
            self._rigid_prims_initialized = False
            self.part_type_by_prim_path = {"/Root/Part_A": "part_a", "/Root/Part_B": "part_b"}
            for p in target_paths_A:
                self.part_type_by_prim_path[p] = "part_a"
            for p in target_paths_B:
                self.part_type_by_prim_path[p] = "part_b"
            print(f"[SceneBuilder] Task2 Cloner: {len(self.parts_prim_paths)} 个零件路径: {self.parts_prim_paths}")
    ...
    def get_box_joints(self) -> np.ndarray:
        """返回 Task4 箱子 articulation 当前关节角。

        评测需要实时折叠状态；初始关节角只用于 reset，不能作为评分输入。
        """
        if not hasattr(self, 'box_articulation') or self.box_articulation is None:
            raise RuntimeError("Task4 box_articulation 未初始化，无法获取盒子关节角")
        if not hasattr(self.box_articulation, 'get_joint_positions'):
            raise RuntimeError("Task4 box_articulation 不支持 get_joint_positions")

        joints = self.box_articulation.get_joint_positions()
        if joints is None:
            raise RuntimeError("Task4 box_articulation.get_joint_positions() 返回 None")

        joints = np.asarray(joints, dtype=np.float32).reshape(-1)
        if len(joints) == 0:
            raise RuntimeError("Task4 盒子关节角为空")
        return joints
